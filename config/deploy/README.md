...
- manager Deployment
- webhook Service and ValidatingWebhookConfiguration
...
- `scripts/deploy.sh` generates a self-signed CA and serving certificate for the webhook, stores the serving keypair in `sandbox-operator-webhook-server-cert`, then patches `caBundle` into the ValidatingWebhookConfiguration.
